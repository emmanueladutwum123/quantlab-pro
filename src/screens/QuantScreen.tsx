...
// Simulated price series for demonstration
...
function HurstTool() {
  ...
  return (
    <>
      <Card title="Hurst Exponent (R/S Analysis)" subtitle="Measures persistence / anti-persistence" accentColor="#00BCD4">
        <SliderRow label="Data Points (N)" value={n} min={50} max={500} step={10}
          format={v => v.toFixed(0) + ' bars'} onChange={setN} />
        <SliderRow label="Trend Strength" value={trend} min={-0.001} max={0.001} step={0.0001}
          format={v => (v * 100).toFixed(3) + '%/bar'} onChange={setTrend} />
        <SliderRow label="Volatility" value={vol} min={0.001} max={0.05} step={0.001}
          format={v => (v * 100).toFixed(1) + '%'} onChange={setVol} />
      </Card>
      ...
      <Card title="Result" accentColor={regimeColor}>
        <View style={styles.hurstGauge}>
          <View style={styles.hurstTrack}>
            <View style={[styles.hurstFill, {
              width: `${result.hurst * 100}%` as any,
...
function GARCHTool() {
  ...
  return (
    <>
      <Card title="GARCH(1,1) Volatility" subtitle="σ²_t = ω + α·ε²_{t-1} + β·σ²_{t-1}" accentColor="#00BCD4">
        <SliderRow label="Data Points (N)" value={n} min={50} max={500} step={10}
          format={v => v.toFixed(0)} onChange={setN} />
        <SliderRow label="α (ARCH term)" value={alpha} min={0.01} max={0.3} step={0.01}
          format={v => v.toFixed(2)} onChange={setAlpha} />
        <SliderRow label="β (GARCH term)" value={beta} min={0.5} max={0.99} step={0.01}
          format={v => v.toFixed(2)} onChange={setBeta} />
        <SliderRow label="True Volatility" value={vol} min={0.001} max={0.05} step={0.001}
          format={v => (v * 100).toFixed(2) + '%'} onChange={setVol} />
      </Card>

      <Card title="Model Output" accentColor="#00BCD4">
      ...
      <Card title="Conditional Volatility (last 40 obs)" accentColor="#00BCD4">
...
function OUTool() {
  ...
  return (
    <>
      <Card title="Ornstein-Uhlenbeck Estimator" subtitle="dX = θ(μ - X)dt + σdW" accentColor="#00BCD4">
        <SliderRow label="Data Points (N)" value={n} min={30} max={500} step={10}
          format={v => v.toFixed(0) + ' obs'} onChange={setN} />
        <SliderRow label="Long-Run Mean (μ)" value={mean} min={50} max={200} step={1}
          format={v => v.toFixed(0)} onChange={setMean} />
        <SliderRow label="Mean-Reversion Speed" value={speed} min={0.01} max={0.3} step={0.01}
          format={v => v.toFixed(2) + '/period'} onChange={setSpeed} />
      </Card>

      <Card title="Estimated Parameters" accentColor="#00BCD4">
        ...
        <MetricRow label="Current Z-Score"     value={result.z_score.toFixed(4)}
          valueColor={zColor} mono />
      </Card>
      ...
      <Card title="Trading Signal" accentColor={zColor}>
        ...
        <Text style={[styles.interp, { marginTop: 8 }]}>
          {`Half-life: ${result.half_life.toFixed(1)} periods — price is expected to revert halfway to μ=${result.mu.toFixed(2)} in ${result.half_life.toFixed(1)} bars.`}
        </Text>
      </Card>
    </>
  )
...
function PerformanceTool() {
  ...
  return (
    <>
      <Card title="Strategy Performance Ratios" subtitle="Annualised · Risk-adjusted" accentColor="#00BCD4">
        <SliderRow label="Sample Size (N)" value={n} min={30} max={500} step={10}
          format={v => v.toFixed(0) + ' trades'} onChange={setN} />
        <SliderRow label="Avg Return / Trade" value={mu} min={-0.002} max={0.003} step={0.0001}
          format={v => (v * 100).toFixed(4) + '%'} onChange={setMu} />
        <SliderRow label="Return Volatility" value={vol} min={0.001} max={0.05} step={0.001}
          format={v => (v * 100).toFixed(2) + '%'} onChange={setVol} />
        <SliderRow label="Risk-Free Rate" value={rfr} min={0} max={0.10} step={0.005}
          format={v => (v * 100).toFixed(1) + '%'} onChange={setRfr} />
      </Card>

      <Card title="Ratios" accentColor="#00BCD4">
        <MetricRow label="Annualised Return" value={`${(result.annualised_return * 100).toFixed(2)}%`}
          valueColor={result.annualised_return > 0 ? colors.green : colors.red} mono />
        <MetricRow label="Annualised Volatility" value={`${(result.annualised_vol * 100).toFixed(2)}%`} mono />
        <MetricRow label="Sharpe Ratio" value={result.sharpe.toFixed(4)}
          valueColor={sharpeColor} mono />
        <MetricRow label="Sortino Ratio" value={result.sortino.toFixed(4)}
          valueColor={sortinoColor} mono />
        <MetricRow label="Calmar Ratio" value={result.calmar.toFixed(4)}
          valueColor={result.calmar > 1 ? colors.green : colors.red} mono />
        <MetricRow label="Max Drawdown" value={`${(result.max_drawdown * 100).toFixed(2)}%`}
          valueColor={result.max_drawdown < 0.1 ? colors.green : result.max_drawdown < 0.2 ? colors.yellow : colors.red} mono />
        <MetricRow label="VaR (95%, 1 trade)" value={`${(result.var_95 * 100).toFixed(4)}%`} mono />
        <MetricRow label="CVaR / ES (95%)" value={`${(result.cvar_95 * 100).toFixed(4)}%`}
  ...
  return (
    <View style={styles.sliderRow}>
      ...
      <Slider minimumValue={min} maximumValue={max} step={step} value={value}
        ...
        minimumTrackTintColor="#00BCD4" maximumTrackTintColor={colors.cardBorder}
...
const styles = StyleSheet.create({
  ...
  toolBtn: { width: '47%', paddingVertical: 9, borderRadius: radius.sm,
             borderWidth: 1, borderColor: colors.cardBorder, alignItems: 'center' },
  toolBtnActive: { backgroundColor: '#00BCD4' + '22', borderColor: '#00BCD4' },
  toolBtnText: { color: colors.textMuted, fontSize: 11, fontWeight: '600' },
  toolBtnTextActive: { color: '#00BCD4' },
  sliderRow: { marginBottom: 8 },
  sliderLabel: { color: colors.textMuted, fontSize: 12 },
  sliderValue: { color: '#00BCD4', fontSize: 12, fontWeight: '700', fontFamily: 'monospace' },
